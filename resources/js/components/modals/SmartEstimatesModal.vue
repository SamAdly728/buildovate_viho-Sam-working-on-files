<template>
    <div class="modal fade" id="smartEstimateModal" tabindex="-1" role="dialog" aria-labelledby="smartEstimateModal" aria-modal="true" style="overflow: hidden;">
        <div class="modal-dialog modal-fullscreen" role="document">
            <div class="modal-content" style="margin: 0 !important; padding: 0 !important;">
                <div class="modal-body p-0 m-0">
                    <div class="card p-0 m-0" style="overflow: hidden; height: 100%;">
                        <div class="card-header border">
                            <h5>{{ estimateFormName ? estimateFormName : 'Loading...' }}</h5>
                            <button class="btn-close" type="button" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="card-body p-0 d-flex m-0">
                            <div class="flex-grow-1 slim-scroll-light"  style="max-height: calc(100vh - 127.6px); overflow-y: auto;" v-if="!loading">
                                <draggable :list="list2" item-key="id" group="fields" @update:list="list2 = $event" :disabled="true">
                            ...
                            <div class="d-flex flex-column align-items-center mt-5 mb-5" style="width: 100%;" v-else>
                                <div style="height: 40px;">
                                    <LoadingStyleMetric></LoadingStyleMetric>
                                </div>
                                <a class="f-w-600 mt-3">Retrieving Form Fields...</a>
                            </div>
                            <div style="width: 400px;" class="d-flex flex-column b-l-light">
                                ...
                                <div class="flex-grow-1 slim-scroll-light" style="overflow-y: auto; max-height: calc(100vh - 462.2px);">
...
function updateModalVisibility() {
    const modalElement = document.getElementById('smartEstimateModal');
    const modalInstance = new bootstrap.Modal(modalElement, {backdrop: 'static', keyboard: false});

    if (store.isModalOpen) {
        modalInstance.show();
        populateForm(store.estimated_id);
    } else {
        document.querySelector(`#smartEstimateModal .btn-close`).click();
    }
...
function handleAttachedToProposal(){
  if(selectedItems.value.length != 0){
    store.selectedItems = selectedItems.value.map(field => {
      return field.data;
    });
    document.querySelector(`#smartEstimateModal .btn-close`).click();
  }else{
