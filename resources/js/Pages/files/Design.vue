          ...
          <div class="file-content" >
            <div class="card">
              ...
              <div class="card-body file-manager ">
                <div class="table-responsive overflow-x-hidden overflow-y-auto custom-scrollbar" v-if="listView" style="max-height: 700px; min-height: auto">
    ...
    <div class="modal-dialog modal-xl" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title text-muted" id="exampleModalLabel">Upload Files - Designs</h5>
                ...
                <div class="card rounded">
                  ...
                  <div class="overflow-auto" >
                    ...
                    <div id="footer-display">
                      <div v-for="design in designsFile">
                        <div v-if="wasUploadedJustNow(design.created_at)">
                          <hr class="mt-0 mb-0">
                          <div class="p-3">
                            <div class="row new-added" :data-id="design.user_file_upload_id">
                              <div class="d-flex align-items-center">
                                <i class="icofont icofont-check text-success mr-2 h4"></i>
                                <img :src="design.s3Url" alt="Uploaded Image" class="img-thumbnail mr-3" style="width: 50px; height: 50px;">
                                <span style="margin-right: 3rem" class="text-muted">{{ truncateFileName(design.file_name) }}</span>
                                <span style="margin-right: 4rem" class="text-muted"><small>{{ Math.trunc(Math.round(design.file_size / 1024 * 100) / 100) + ' KB' }}</small></span>
                                <button type="button" class="txt-danger" style="position: absolute; right: 18px" @click="deleteSinglePhoto(design.user_file_upload_id)">Remove</button>
                              </div>
                            </div>
                          </div>
                        </div>
...
<!-- Move to modal -->
  <div class="modal fade" id="move-to" tabindex="-1" role="dialog" aria-labelledby="moveTo" aria-hidden="true">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title text-muted" id="moveTo">Move to</h5>
          <button class="btn-close" type="button" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">
          <select class="form-select digits" id="move-to-folder" v-model="moveToFolder">
        ...
        <div class="modal-footer">
          ...
          <button class="btn btn-secondary" type="button" @click="saveMoveTo">Save</button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    async saveMoveTo() {
      if (!this.moveToFolder) {
        ...
        try {
          ...
          await axios.post('/files/move-to', {'move-to-folder': this.moveToFolder, 'file-ids': this.selectedItems.selectedFileId}).then(response => {
    ...
    toggleSelectAll() {
      this.selectedItems.selectedFileId = this.selectAll
          ? this.designsFile.map(item => item.user_file_upload_id)
          : [];
      this.$emit('selection-changed', this.selectedItems.selectedFileId);
    },
    checkSelectAllStatus() {
      this.selectAll = this.selectedItems.selectedFileId.length === this.designsFile.length;
    },
    deleteFile() {
      if (this.selectedItems.selectedFileId.length === 0) {
        toast.error('No file has been selected!')
  ...
  mounted() {
    ...
    // };
    //
    // let dz = new Dropzone('#designFiles')
    //
