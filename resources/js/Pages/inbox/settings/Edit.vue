         ...
         <div class="col-md-12">
            <div class="card">
               <div class="card-header pb-0">
                  <h5>Update Inbox Settings: <span class="text-success">{{ form.name }}</span></h5>
                  <hr>
               </div>
               <div class="card-body">
                  <div class="row">
...
export default{
   props:(['setting']),
   data(props) {
      const sett = props.setting;
      console.log('sett',sett.name);
      return {
         form: {
            id: sett.id,
            name: sett.name,
            phone_number: sett.phone_number,
            do_not_disturb: (sett.do_not_disturb==1) ? true : false,
            call_forwarding: (sett.call_forwarding==1) ? 'invoice' : 'number'
         }
      }
   },
   methods:{
