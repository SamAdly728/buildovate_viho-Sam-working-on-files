              ...
              <template #item="{element}">
                <div class="item-card" style="cursor: pointer; overflow: hidden;">
                  ...
                  <div class="item-body">
                    ...
                    <div class="item-subtitle">{{ element.status  }}</div>
                    <div class="item-info">
                  ...
                  <div class="item-footer">
                    <div></div>
                    <div>
                      <span class="item-time">{{ formatDate(element.created_at)  }}</span>
  ...
  methods: { 
    ...
    onItemMoved(event) {
      if(event.added){
        ...
        this.moveLead(item.proposal_id,destinationData.id);
      }
    },
    getLeadsGroupedByStage() {
      axios.get('/api/getProposalsGroupedByStage')
        .then(response => {
    ...
    moveLead(proposalId, newStageId) {
      axios.patch(`/api/proposals/${proposalId}/stage`, { stage_id: newStageId })
        .then(response => {
          console.log(response.data.message);
        })
        .catch(error => {
          console.error('Error updating proposal stage:', error.response);
        });
    },
    handleNewLeadClick() {
